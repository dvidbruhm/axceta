...
for location in locations:
    temp_df = df[df["LocationName"] == location].copy()
    temp_result = temp_df

    # Reorder table by date to make sure it is always correct
    temp_df["AcquisitionTime"] = pd.to_datetime(temp_df["AcquisitionTime"])
    temp_df = pd.DataFrame(temp_df.sort_values(by="AcquisitionTime"))
    ...
    values_to_smooth = temp_df[col_name].values
    acquisition_time = temp_df["AcquisitionTime"].values
